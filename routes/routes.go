...
import (
	...
	"github.com/nabilwafi/warehouse-management-system/middlewares"
)
...
func (r *router) Start(port string) {
	v1 := r.router.Group("/api/v1")
	{
		...
		products := v1.Group("/products")
		{
			...
			products.GET("/:id", middlewares.RoleMiddleware("staff", "admin"), r.product.GetProductByID)
			products.PUT("/:id", middlewares.RoleMiddleware("admin"), r.product.UpdateProduct)
			products.DELETE("/:id", middlewares.RoleMiddleware("admin"), r.product.DeleteProduct)
		}
		...
		orders := v1.Group("/orders")
		{
			...
			orders.GET("/:id", middlewares.RoleMiddleware("admin", "staff"), r.order.GetOrderByID)
		}
	}
